...
IMPORTANT RULES:
- Only write SELECT statements. Never write INSERT, UPDATE, DELETE, DROP, or any DDL.
- Always use schema prefixes: dim.Customer, fact.ARInvoice etc.
- For date filtering use the Date column in dim.Date or date columns directly on fact tables.
- For "current year" use: WHERE d.IsCurrentYear = 1 (joining to dim.Date)
- For "current month" use: WHERE d.IsCurrentMonth = 1
- For "today" use: WHERE d.IsToday = 1
- For "year to date" use: WHERE d.Year = YEAR(GETDATE()) AND d.Date <= CAST(GETDATE() AS date)
- AgingBucket on fact.ARInvoice and fact.APInvoice uses these exact values: 'Current', '1-30', '31-60', '61-90', '90+'
- BalanceDue > 0 means the invoice is still open/unpaid
- Always TOP 100 results unless the user asks for a specific number or a total/aggregate
- Format currency values with 2 decimal places in the query using ROUND(col, 2)
- Keep queries simple and readable — no CTEs unless absolutely necessary
`;
...
chartType rules:
- "number" — single aggregate value (total revenue, count of invoices, etc.)
- "bar" — comparison across categories (revenue by customer, top parts, etc.)
- "table" — detailed rows (list of invoices, customer details, etc.)
...
Return ONLY the JSON object. No markdown, no explanation outside the JSON.`,
    ...
    return new Response(JSON.stringify({
      ...
      note: "SQL ready — Fabric execution coming in next phase"
    }), {
      status: 200,
      headers: { "Content-Type": "application/json" }
    });
    ...
    return new Response(JSON.stringify({ error: "AI query failed", detail: String(err) }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
